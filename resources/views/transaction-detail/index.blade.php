...
<div class="container">
    <h3 class="text-center text-secondary">DATA TRANSACTION DETAIL
    </h3>
    ...
    <div class="card">
        <div class="card-header">
            <h1 class="text-center">TABEL TRANSACTION DETAIL</h1>
            ...
            <div class="table-responsive">
                <table class="table table-striped" id="example2">
                    ...
                    <tbody class="text-center">
                        ...
                        <tr>
                            <td>{{$no++}}</td>
                            <td>{{$dttd->user->customer->first_name}} {{$dttd->user->customer->last_name}}</td>
                            <td>@foreach ($dttd->product_transaction_detail as $dtptc)
                                {{$dtptc->product->name}},
                                @endforeach
                            </td>
                            <td>{{$dttd->total_price}}</td>
                            <td>{{$dttd->pembayaran}}</td>

                            <td>
                                <a href="/transaction-detail/detail-index/{{$dttd->id}}" class="btn btn-sm btn-primary"><i class="fas fa-file-alt"></i></a>
